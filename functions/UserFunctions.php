...
function sap_check_user_payment_status(){
	...
	if( !empty( $user ) && $user['role'] != 'superadmin'){
		...
		$str_time 		= strtotime($result->expiration_date);
		
		if( ( ( $result->membership_status != '1' &&  $result->membership_status != '3' ) || ( !empty( $str_time ) && date('Y-m-d') > date('Y-m-d', $str_time ) ) ) &&  ( $is_payment <= 0 ) ){
