...
jQuery(document).ready(function ($) {
   ...
   $('form#linkedin-settings').on('submit', function(event) {
    ...
    $('form#twiiter-settings').validate({
    	errorElement: "em",errorPlacement: function (error, element) {error.addClass("help-block");element.parents(".col-sm-6").addClass("has-error");},
        success: function (label, element) {},
        highlight: function (element, errorClass, validClass) {
            ...
            if ( parent.find("label.error").length === 0 ) {
                parent.append('<label id="' + element_name + '-error" class="error twiiter-settings-error">This field is required.</label>');
            }
        },
        unhighlight: function (element, errorClass, validClass) {
            ...
            var parent = $(element).parent();
            if ( parent.find("label.error").length > 0 ) {
                var error = parent.find("label.error");
                error.remove();
            }
        },
        submitHandler: function(form) {$('form#twiiter-settings').find(':submit').prop("disabled", true);form.submit();},
    });
