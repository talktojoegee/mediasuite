			...
			<?php if (!isset($match['params']['id'])) { ?>
				<form action="<?php echo $router->generate('quick_save_post'); ?>" id="quick-post-form" class="quick-post" method="post" enctype="multipart/form-data" accept-charset="utf-8">
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 pull-left">
						...
						<div class="col-lg-12">
							<div class="row">
								...
								<div class="col-lg-12 quick-image-wrap">
									...
									<div class="form-group">
										<?php if (!empty($post_data->image)) { ?>
											<input id="quick-post-image" tabindex="3" value="" name="image" type="file" class="file file-loading" data-show-upload="false" data-show-caption="true" data-allowed-file-extensions='["png", "jpg","jpeg", "gif"]' data-max-file-size="1024K" data-initial-preview="<img src='<?php echo SAP_IMG_URL . $post_data->image; ?>' class='uploaded-img'/>">
										<?php } else { ?>
											<input id="quick-post-image" tabindex="3" value="" name="image" type="file" class="file file-loading" data-show-upload="false" data-show-caption="true" data-max-file-size="1024K" data-allowed-file-extensions='["png", "jpg","jpeg", "gif"]'/>
										<?php } ?>
								...
								<div  class="col-lg-12 quick-video-wrap">
									...
									<input id="quick-post-video" tabindex="3" value="" name="video" type="file" class="file file-loading" data-show-upload="false" data-show-caption="true" data-max-file-size="1024K" data-allowed-file-extensions='["mp4"]'/>
								</div>
							...
							<div class="row">
								<div class="col-lg-12">
									<div class="form-group sap-share-link-parent" >
										<label class="control-label" id="sap-share-link-parent-link"><?php echo $sap_common->lang('link'); ?></label>
										<input class="form-control sap-quick-valid-url" tabindex="4" placeholder="Content share link" name="share_link" value="<?php echo!empty($post_data->share_link) ? $post_data->share_link : ''; ?>">
									</div>
								</div>
							</div>
						</div>
						...
						<div class="col-lg-12">
							<div class="row">
								<div class="col-lg-12 buttons buttons-display-flex button-amin-wrap">
									<div class="col-lg-6">
										<div class="form-group">
											<label for="status" class="col-sm-4 control-label"><?php echo $sap_common->lang('schedule'); ?></label>
											<div class="col-sm-8">
												<i class="fa fa-clock-o sap-schedule-icon" aria-hidden="true" <?php echo!empty($post_data->status) && $post_data->status == 2 ? 'style="display: none;"' : ''; ?>></i>
												<input type="text" name="sap-schedule-time" <?php echo!empty($sap_schedule_time) ? 'value="' . date('Y-m-d H:i', $sap_schedule_time) . '"' : ''; ?> readonly="" class="form-control sap-datetime sap-schedule-input" <?php echo!empty($post_data->status) && $post_data->status == 2 ? 'style="display: block;"' : ''; ?>>
											</div>
										</div>
					...
					<?php if (!empty($status_meta)) {?>
						<div class="sap-quick-post-privew-wrap">
							...
							foreach ($status_meta as $status_meta_key_ => $status_meta_value) {	
								...
								$scheduled_date = '';
								$social_class = "sap-quick-post-privew-" . $status_meta_key;
								...
								if ($get_status === '1' || $get_status === 1) {
									...
									$status = "Scheduled";
									$scheduled_date = date("F j, Y g:i a", $sap_schedule_time);
									...
									<div class="sap-quick-post-privew-content">
										...
										<?php if (!empty($preview_image)) {  ?> <div class="sap-quick-post-privew-image">
											<?php if( !empty( $social_link_to_post ) ){ 
												echo '<a href="'.$social_link_to_post.'" target="_blank">';} ?><img src="<?php echo $preview_image; ?>">
