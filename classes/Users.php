...
class SAP_Users {
	...
	public function forgot_password_process() {
		...
		if ( !empty( $result ) && count( $result ) > 0 ) {
			...
			$to = $result[0]->email;
			$subject = "Reset your Mingle - " . SAP_NAME . " password";
		...
		if ( isset($_POST['form-submitted']) ) {
			...
			if ( $this->_db->insert($this->_table_users, $member_data) ) {
				...
				/*if(isset($_POST['gateway_type']) && !empty($_POST['gateway_type']) && $_POST['gateway_type'] == 'stripe'){
					...
					if(!empty($stripe_payment_result->id) && !empty($stripe_payment_result->next_action)){
						
						$redirect 	  = $stripe_payment_result->next_action;
						$redirect_url = $redirect->redirect_to_url;
				...
				if( !isset( $_POST['gateway_type'] ) ){ //For Free Plan
					...
					$insert_payment = array(
						...
						'amount' 		=> isset( $_POST['amount'] ) ? trim($this->_db->filter($_POST['amount'])) : '0',
						'type' 			=> '0',
						'gateway' 		=> 'paystack',
						'payment_status'=> '1',
						'transaction_id'=> isset( $_POST['transaction'] ) ? trim($this->_db->filter($_POST['transaction'])) : '',
						'transaction_data' => '',
						'created_date' 	=> date('Y-m-d H:i:s'),
						'modified_date'     => date('Y-m-d H:i:s'),
					);
				...
				if( isset($_POST['gateway_type'] )  && $_POST['gateway_type'] == 'stripe' && empty( $stripe_payment_result )){
