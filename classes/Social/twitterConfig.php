...
class SAP_Twitter {
    ...
    public function sap_post_to_twitter($post_id) {
        ...
        if (!empty($post_desc))
            $post_desc = $this->posts->sap_limit_character($post_desc, 278);
            ...
            foreach ($accounts as $key => $value) {
                ...
                try {
                    ...
                    if (isset($result->id) && !empty($result->id)) {
                        ...
                        $this->flash->setFlash( 'Twitter : Post sucessfully posted on - '.$result->user->name, 'success','',true );
                        $this->sap_common->sap_script_logs('Twitter : Post sucessfully posted on - ' . $result->user->name, $user_id );
