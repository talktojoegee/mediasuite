...
class SAP_Instagram {
    ...
    public $fbPermissions = ['pages_manage_posts','pages_show_list','instagram_basic','instagram_content_publish'];
    public $facebook, $settings, $flash, $posts, $common,$instagram;
    ...
    public function sap_fetch_instagram_account_usernames( $all_user_ids , $fb_access_token ){
        ...
        if( !empty( $all_user_ids ) ) {
            ...
            foreach( $all_user_ids as $key => $user_data ) {
                    ...
                    $get_api = 'https://graph.facebook.com/v13.0/'.$user_data.'?fields=name,username&access_token='.$fb_access_token;
    ...
    public function sap_fb_fetch_instagram_accounts( $facebook_app_id  ){
        ...
        if(!empty($_SESSION['sap_insta_user_accounts']['auth_accounts']) && is_array($_SESSION['sap_insta_user_accounts']['auth_accounts'])){
            ...
            foreach( $_SESSION['sap_insta_user_accounts']['auth_accounts'] as $page_id => $page_data ){
                   ...
                   $get_api = 'https://graph.facebook.com/v3.2/'.$page_id.'?fields=instagram_business_account&access_token='.$fb_access_token;
                    curl_setopt_array($curl, array(
                    CURLOPT_URL => $get_api,
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_ENCODING => '',
        ...
        curl_close($curl);        
        $all_intagram_user_ids = array_unique( $all_intagram_user_ids );
        $all_instagram_details = $this->sap_fetch_instagram_account_usernames($all_intagram_user_ids,$fb_access_token);
...
// Taking user auth tokens
        if (isset($_GET['wpw_auto_poster_insta_app_method']) && $_GET['wpw_auto_poster_insta_app_method'] == 'appmethod') {
            $user_auth_tokens = $_SESSION['sap_insta_user_id'];
        } else {
            $user_auth_tokens = isset($_GET['fb_app_id']) ? $_GET['fb_app_id'] : '';
        }

        $api['auth_accounts'][$_SESSION['sap_insta_user_id']] = $this->_user_cache['name'] . " (" . $_SESSION['sap_insta_user_id'] . ")";

        if (isset($_GET['wpw_auto_poster_insta_app_method']) && $_GET['wpw_auto_poster_insta_app_method'] == 'appmethod') {
            $api['auth_tokens'][$_SESSION['sap_insta_user_id']] = $this->grantaccessToken;
        } else {
    ...
    public function sap_get_pages_tokens($user_id='') {
        ...
        try {
            ...
            if (isset($_GET['wpw_insta_grant']) && $_GET['wpw_insta_grant'] == 'true' && $_GET['wpw_auto_poster_insta_app_method'] == 'appmethod') {
                ...
                $ret = $this->facebook->get('/me/accounts/', $this->grantaccessToken);
        ...
        if( !empty( $sap_instagram_sess_data ) ) {
            ...
            if (!empty($accounts)) { // Check all user ids
                foreach ($accounts as $key => $value) {
                    try {
                        if ($post->share_link != '') {
                            $posting_logs_data['link'] = $post->share_link;
                        }
